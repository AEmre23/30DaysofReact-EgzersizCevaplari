...
const Cats = ({ allCats, catLength }) => {
  ...
  let digits = metricWeigth.replace(/\D/g, ""); // to avoid "-" in object

  if (digits.length > 2){
    //if its like 4-11 kg ,which make them 3 length,
    // it makes  411 so we have to seperate second number like 4 and 11 to sum up.
      ...
      <div className='App'>
        ...
        <div>
          ...
          <div className='countries-wrapper'>
              <Cats allCats={catData} catLength={numberOfCatBreeds} />
          </div>
        </div>
      </div>
    )
